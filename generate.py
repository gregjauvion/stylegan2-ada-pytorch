...
def slerp(val, low, high):
    ...
    if len(low.shape) == 2:
        ...
        for i in range(low.shape[0]):
            omega = np.arccos(
                np.clip(np.dot(low[i] / np.linalg.norm(low[i]), high[i] / np.linalg.norm(high[i])), -1, 1))
            so = np.sin(omega)
            if so == 0:
...
@click.option('--process',
              type=click.Choice(['image', 'neighbors', 'interpolation', 'truncation', 'interpolation-truncation']),
...
@click.option('--diameter', type=float, help='Distance around seed to sample from (neighbors process)', default=0.1)
@click.option('--num_samples', type=int, help='Number of neighbors to generate (neighbors process)', default=10)
...
def generate_images(
    ...
    if (process == 'neighbors'):
        ...
        for seed_idx, seed in enumerate(seeds):
            print('Generating neighbors for seed %d (%d/%d) ...' % (seed, seed_idx, len(seeds)))
